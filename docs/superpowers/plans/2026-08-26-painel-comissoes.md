...
- [ ] **Step 1:** aplicar a migração `ume_comissoes` com o SQL da spec (duas tabelas; UNIQUE (mes, origem, retailer_id); PK composta na meta).
- [ ] **Step 2:** verificar com `select` vazio que as tabelas existem.
...
- [ ] **Step 1:** adicionar `description: string` à interface e `description: (o.description as string) ?? ''` ao mapeamento.
...
- [ ] **Step 1:** `npm i xlsx`.
- [ ] **Step 2:** criar `lib/comissoes.ts` com:
...
- [ ] **Step 3:** script temporário que roda `parsePlanilhaUme` na planilha real de julho: 264 linhas de varejo, total = 39.704,92 (bate), mês '2026-07'.
- [ ] **Step 4:** commit.
...
- [ ] **Step 1:** `POST` multipart (`req.formData()`), aceita 1+ arquivos `.xlsx` ≤ 2 MB; para cada um: `parsePlanilhaUme` → delete `(mes, origem)` → insert linhas + upsert meta. Resposta com resumo por arquivo (mes, origem, linhas, total). Erro de validação → 422 com a mensagem do parser. Protegida pelo middleware (Task 7) — sem auth própria.
- [ ] **Step 2:** typecheck.
...
- [ ] **Step 1:** `POST { opportunityId, retailerId }` → `getOpportunity`; se descrição já tem `UME_RID:` com valor ≠ → 409; senão `postEvo('/int/updateOpportunity', { id, description: desc ? desc + ' | UME_RID: N' : 'UME_RID: N' })` (nunca enviar `tags`). Como `post` do evotalks é privado, exportar helper `updateOpportunityDescription(id, desc)` em `lib/evotalks.ts`.
- [ ] **Step 2:** typecheck.
...
- [ ] **Step 1:** baixar via Gmail (RAW) o anexo "Comissao ... FCDL - Julho 2026.xlsx" (msg 1a001810cd8e96f4) pro scratchpad.
- [ ] **Step 2:** rodar o import local (dev server ou script direto com o parser + supabase) das duas planilhas de julho.
- [ ] **Step 3:** conferir no banco: meta carteira 39.704,92; meta fcdl 2.986,76; NF total 42.691,68 (bate com o email do Gerisson).
...
- [ ] **Step 1:** página server-side: seletor de mês, abas AIVA/UME (?tab=), toggle Carteira|FCDL dentro de UME (?origem=), cards (Contratos/Originação/MDR/Comissão da aba + card Valor da NF), cards-filtro por estado, tabela com busca `casaBusca`, cabeçalho fixo (padrão thFixo do /desempenho), coluna Data Studio na aba AIVA, LeadDrawer/ClickableRow quando houver lead (casar CNPJ→lead como no /desempenho).
- [ ] **Step 2:** typecheck + build.
- [ ] **Step 3:** commit.

### Task 8: verificação com julho real + deploy

- [ ] **Step 1:** rodar a conferência de julho via script (mesma função `conferir`) e comparar com a lista do email do Aldo (anexo "Sem-Comissao-Julho26-Funil11.xlsx", msg 1a01630068be4026): as lojas apontadas por ele devem cair em 💤/⚠️.
- [ ] **Step 2:** conferir os 4 estados: contagens plausíveis (✅ ≈ 240 vinculadas com venda, 🆕 deve pegar IVS/lojas sem conta).
- [ ] **Step 3:** commit final + push (deploy Vercel automático).
- [ ] **Step 4:** smoke em produção: abrir /comissoes (redireciona pra login = middleware ok) e conferir a página logado por conta do Aldo (ele mesmo valida visual).
